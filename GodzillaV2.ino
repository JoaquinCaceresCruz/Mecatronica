...
// Incliye las variables generales para la Tira LED
int pixelpin = 6; //Pin en el que se conecta el LED
int pixeles = 14; //Numero de Pixeles
int intensidad=5; //Variable de intensidad
int contador = 0; //Contador para la variable LED
int intervalo_parpadeo=300; //Intervalo para el parpadeo de carga
...
void setup() {
  ...
  while (contadorsito==1){//Ejecuta la funcion godzilla_roar() hasta que se alcanza la frecuencia maxima definida
  godzilla_roar();
  }
...
void loop() {
  servo1.write(700); //Define un bucle final para godzilla 
  delay(500);
  servo1.write(1500);
  delay(500);
}
...
void Inicio(){
  ...
  servo1.write(1500);
  for(int j=11; j<=pixeles; j++){
      strip.setPixelColor(j,strip.Color(0,128,255)); //Enciende los ojos y la boca
    }
...
void Final(){//Funcion que describe el comportamiento final deseado
  pixeles=14; //Considera todos los pixeles conectador para el rugido
  ...
  for(int i=1; i<=numero_parpadeos;i++){ //
    ...
    for(int j=0; j<pixeles; j++){ //Enciende los LED
      strip.setPixelColor(j,strip.Color(0,128,255));
      servo1.write(800);
    }
    strip.show();
...
void TiraLED(int Contador_LED, int frec, int dur_in){// Define la funcion TiraLED, una subfuncion de godzilla_roar
	for(contador; contador<=Contador_LED;contador++){//secuencia de encendido
		strip.setPixelColor(contador,strip.Color(0,128,255));// Setea el color del led "contador"
		strip.setBrightness(180);//setea la intensidad antes del parpadeo
		strip.show();
    toneAC(frec, intervalo_parpadeo);
    delay(intervalo_parpadeo);
    noToneAC();
    intensidad=5;
    
		for(int contador_parpadeo=contador+1;contador_parpadeo<pixeles;contador_parpadeo++){//setea el parpadeo de las led que no estan encendidas            
			strip.setPixelColor(contador_parpadeo ,strip.Color(0,128,255));
			...
			intervalo_parpadeo=intervalo_parpadeo*0.98;
      toneAC(frec, intervalo_parpadeo);
      delay(intervalo_parpadeo);
      noToneAC();
			}
...
void godzilla_roar(){ //Define la funcion godzilla_roar, consiste en la carga del aliento atomico de godzilla
...
pixeles=11; //Define los pixeles utilizados para la carga
  ...
  while(topF<=maxF){
    for(frec=topF; frec>=botF;frec=frec-(topF-botF)/pixeles){//La idea es que con cada ascenso de frecuencia se encienda un led
    TiraLED(Contador_LED, frec, dur_in);
    }//del for "frec"
...
for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
    ...
    strip.setPixelColor(ledsita ,strip.Color(0,128,255));
