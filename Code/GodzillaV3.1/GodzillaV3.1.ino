...
int TRIG = 7 ;
int ECO = 8  ;
int RANGO_BUSQUEDA=100; //Rango o distancia de busqueda del sensor
int DISTANCIA;
...
int DeteccionEnemigo(){
  ...
  if(Detectado==1){Distancia_Deteccion=Promedio;} //si se cumplen las condicion entrega el valor de la distancia detectada
...
void TiraLED(int Contador_LED2, int frec, int dur_in, int DISTANCIA){// Define la funcion TiraLED, una subfuncion de godzilla_roar
  int variacion_color = map(DISTANCIA,0,RANGO_BUSQUEDA,255,128);// Ya agregada
  int variacion_parpadeo = map(DISTANCIA,0,RANGO_BUSQUEDA,intervalo_parpadeo,250); // Aun por agregar
  
	for(Contador_LED1; Contador_LED1<=Contador_LED2;Contador_LED1++){//secuencia de encendido
		strip.setPixelColor(Contador_LED1,strip.Color(0,variacion_color,255));// Setea el color del led "contador"
    ...
    intensidad=5;
		...
		for(int contador_parpadeo=Contador_LED1+1;contador_parpadeo<pixeles;contador_parpadeo++){//setea el parpadeo de las led que no estan encendidas            
			strip.setPixelColor(contador_parpadeo ,strip.Color(0,variacion_color,255));
...
pixeles=11; //Define los pixeles utilizados para la carga
  ...
  while(topF<=maxF){
    ...
    for(frec; frec>=botF;frec=frec-(topF-botF)/pixeles){//La idea es que con cada ascenso de frecuencia se encienda un led
