...
int DeteccionEnemigo(){
  ...
  Serial.println(Distancia_Deteccion);
  return Distancia_Deteccion;
}


void Final(int Distancia_Deteccion){//Funcion que describe el comportamiento final deseado
  int variacion_color = map(Distancia_Deteccion,0,RANGO_BUSQUEDA,200,0);
  int variacion_color2 = map(Distancia_Deteccion,0,RANGO_BUSQUEDA,30,255);
...
void TiraLED(int Contador_LED2, int frec, int dur_in, int Distancia_Deteccion){// Define la funcion TiraLED, una subfuncion de godzilla_roar
  Serial.print("variacion de color");
  
  int variacion_color = map(Distancia_Deteccion,0,RANGO_BUSQUEDA,255,0);
  int variacion_color2 = map(Distancia_Deteccion,0,RANGO_BUSQUEDA,20,255);
  int variacion_parpadeo = map(Distancia_Deteccion,0,RANGO_BUSQUEDA,150,intervalo_parpadeo);
  
	for(Contador_LED1; Contador_LED1<=Contador_LED2;Contador_LED1++){//secuencia de encendido
    Serial.println("lento 1");
		strip.setPixelColor(Contador_LED1,strip.Color(variacion_color,0,variacion_color2));// Setea el color del led "contador"
    ...
    intensidad=5;
      ...
      strip.setPixelColor(contador_parpadeo ,strip.Color(variacion_color,0,variacion_color2));
...
void godzilla_roar(int Distancia_Deteccion){ //Define la funcion godzilla_roar, consiste en la carga del aliento atomico de godzilla
...
pixeles=11; //Define los pixeles utilizados para la carga
  while(topF<=maxF){
    ...
    TiraLED(Contador_LED2, frec, dur_in, Distancia_Deteccion);
    }//del for "frec"
  ...
  contadorsito+=1;
  Final(Distancia_Deteccion);//Ejecuta la secuencia final del rugido
  }
...
void loop() {
    ...
    contadorsito==1;
    //musica_inicial();
    int Distancia_Enemigo=DeteccionEnemigo();
    if (Distancia_Enemigo>0){
    ...
    Serial.print("DISTANCIA Enemigo = ");
    Serial.println(Distancia_Enemigo);
    godzilla_roar(Distancia_Enemigo);    
